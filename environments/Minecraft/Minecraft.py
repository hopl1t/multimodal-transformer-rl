...
# from scipy.misc import imresize  # Deprecated since scipy 1.0.0 using PIL instead
...
class Minecraft():
    def __init__(self, handle_noise=''):
        ...
        # These are for compatability with cleanrl
        self.metadata = {}
        if Config.USE_AUDIO:
            shape = (2, 84, 84)
        else:
    ...
    def _get_obs(self, show_gt=False, return_agt_loc=False):
        if Config.USE_AUDIO:
            ...
            next_observation = np.stack((image, audio))
        else:
            # next_observation = self._preprocess_img(show_gt = show_gt)
            # First expansion as there are 4 images in cleanrl implementation
            next_observation = np.expand_dims(self._preprocess_img(show_gt=show_gt), 0)
    ...
    # AUDIO-RELATED
    def _init_mfcc(self):
        self.mfcc_target_good = self._get_mfcc('../environments/Minecraft/assets/target_good.wav')
        self.mfcc_target_bad = self._get_mfcc('../environments/Minecraft/assets/target_bad.wav')
        self.mfcc_no_listen = self._get_mfcc('../environments/Minecraft/assets/noise.wav')
    ...
    def _wav_to_mfcc(self, samplerate, wav):
        ...
        mfcc_data = np.array(Image.fromarray(mfcc_data).resize(size=(Config.IMAGE_WIDTH, Config.IMAGE_HEIGHT),
    ...
    def _init_render(self):
        ...
        if self.simple_render:
            ...
            self.img_agent = self._read_img(
                '../environments/Minecraft/TexturePacker/All/Characters/Player_Male/male.png', interp=cv2.INTER_NEAREST)
            self.img_stone = self._read_img('../environments/Minecraft/TexturePacker/All/Tiles/stone.png')
            self.img_pick_gold = self._read_img('../environments/Minecraft/TexturePacker/All/Items/pick_gold.png',
                                                interp=cv2.INTER_NEAREST)
            self.img_pick_iron = self._read_img('../environments/Minecraft/TexturePacker/All/Items/shovel_bronze.png',
                                                interp=cv2.INTER_NEAREST)
            self.img_stone_gold = self._read_img('../environments/Minecraft/TexturePacker/All/Tiles/stone_gold.png')
            self.img_stone_iron = self._read_img('../environments/Minecraft/TexturePacker/All/Tiles/stone_iron.png')
