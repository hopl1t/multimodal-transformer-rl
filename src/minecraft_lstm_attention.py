...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor = layer_init(nn.Linear(129, envs.single_action_space.n), std=0.01)
        self.critic = layer_init(nn.Linear(129, 1), std=1)
    ...
    def get_states(self, x, lstm_state, done):
        ...
        hidden = torch.cat((video_hidden, audio_hidden), dim=1)
        attention_weights = self.attention(hidden)
        attention_weights = torch.sigmoid(attention_weights)
        hidden = video_hidden * attention_weights + \
            audio_hidden * (1-attention_weights)
        ...
        # Add the LSTM weights to the final hidden state to allow the actor and critic nets to use attention as well
        new_hidden = torch.cat((new_hidden, attention_weights), dim=1)
